...
- **Breaking change:** Remove the `AgentType` contract and the `AgentDetails.AgentType`, `AgentDetails.AgentClientIP`, and `InferenceCallDetails.ResponseId` members, with their constructor and deconstruction parameters. The protected `OpenTelemetryScope` constructor now accepts a `Request?` so shared request context is applied by the base scope; custom subclasses must pass this argument ([#165](https://github.com/microsoft/opentelemetry-distro-dotnet/pull/165)).
- Reject execute-tool extension data keys that collide with declared schema fields, preventing untyped values from bypassing field and enum validation ([#165](https://github.com/microsoft/opentelemetry-distro-dotnet/pull/165)).
