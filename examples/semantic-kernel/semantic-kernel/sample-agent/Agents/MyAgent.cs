...
public class MyAgent : AgentApplication
{
        ...
        await A365OtelWrapper.InvokeObservedAgentOperation(
         ...
         async () =>
         {
             ...
             if (!TermsAndConditionsAccepted)
             {
                 ...
                 {
                     var response = _useManualInstrumentation
                         ? await InvokeWithManualInstrumentation(turnContext, new ChatHistory())
                         : await agent365Agent.InvokeAgentAsync(turnContext.Activity.Text, new ChatHistory(), turnContext);
             ...
             if (turnContext.Activity.ChannelId.IsParentChannel(Channels.Msteams))
             {
                 ...
                 {
                     await TeamsManualInstrumentationAsync(turnContext, turnState, cancellationToken);
                 }
                 else
                 {
             ...
             switch (agentNotificationActivity.NotificationType)
             {
                 ...
                 case NotificationTypeEnum.WpxComment:
                     try
                     {
                         ...
                         var response = await agent365Agent.InvokeAgentAsync($"You have received the following Word document content and comments. Please follow refer to these when responding to comment '{commentToAgent}'. {wordContent.Content}", chatHistory, turnContext);
    ...
    /// <summary>
    /// Invokes the ManualInstrumentationAgent which creates InvokeAgentScope, InferenceScope,
    /// and ExecuteToolScope explicitly instead of relying on auto-instrumentation.
    /// </summary>
    private Task<Agent365AgentResponse> InvokeWithManualInstrumentation(ITurnContext turnContext, ChatHistory chatHistory)
    {
        var manualAgent = new ManualInstrumentationAgent(_kernel, _configuration, _logger);
        return manualAgent.InvokeAgentAsync(turnContext.Activity.Text, chatHistory, turnContext);
    }

    /// <summary>
    /// Teams message handler for manual instrumentation mode with streaming.
    /// </summary>
    protected async Task TeamsManualInstrumentationAsync(ITurnContext turnContext, ITurnState turnState, CancellationToken cancellationToken)
    {
        await turnContext.StreamingResponse.QueueInformativeUpdateAsync("Working on a response for you (manual instrumentation)", cancellationToken);
        try
        {
            ChatHistory chatHistory = turnState.GetValue("conversation.chatHistory", () => new ChatHistory());
            var response = await InvokeWithManualInstrumentation(turnContext, chatHistory);
