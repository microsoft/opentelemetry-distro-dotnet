...
using Microsoft.OpenTelemetry.AzureMonitor.AzureSdkCompat;
...
namespace Microsoft.OpenTelemetry
{
    ...
    internal static class OpenTelemetryBuilderExtensions
    {
        ...
        /// </para>
        ///
        /// <para>The following instrumentation is added for distributed tracing:</para>
        /// <list type="bullet">
        ...
        internal static IOpenTelemetryBuilder UseAzureMonitor(this IOpenTelemetryBuilder builder, Action<AzureMonitorOptions> configureAzureMonitor)
        {
            ...
            // Probe options to check SkipExporter
            var probeOptions = new AzureMonitorOptions();
            configureAzureMonitor?.Invoke(probeOptions);

            if (!probeOptions.SkipExporter)
            {
